...
function FilterSidebar({
  ...
  const sections = [
    {
      ...
      content: <div className="pt-3">
          ...
          {categoriesLoading && <p className="mt-2 text-xs text-muted-foreground">Đang tải danh mục...</p>}
        </div>,
    },
    {
      id: "size",
    ...
    },
    {
      id: "price",
      title: "Giá",
      content: <div className="grid grid-cols-2 gap-3 pt-3">
          <input
            type="number"
            min="0"
    ...
    },
    {
      id: "prescription",
      title: "Theo đơn kính",
      content: <div className="flex items-center gap-3 pt-3">
          <Checkbox
          ...
          <div>
            ...
            {prescriptionFilterLocked && <p className="text-xs text-muted-foreground">
  ...
  function toggleSection(sectionId) {
    setOpenSections((currentSections) => currentSections.includes(sectionId)
  ...
  return <aside className="w-56 shrink-0">
      <div className="mb-6">
        <h3 className="text-sm" style={{ fontWeight: 600 }}>Bộ lọc API</h3>
        <button
          type="button"
      ...
      <div className="border-t border-border">
        {sections.map((section) => <div key={section.id} className="border-b border-border py-1">
            <button
              type="button"
            ...
            >
              ...
              <Plus
                className={`w-4 h-4 text-foreground transition-transform ${openSections.includes(section.id) ? "rotate-45" : ""}`}
              />
            </button>
            {openSections.includes(section.id) && section.content}
          </div>)}
      </div>
    </aside>;
}
