...
function ProductListingPage() {
  ...
  const categoryName = category ? category.charAt(0).toUpperCase() + category.slice(1) : "K\xEDnh R\xE2m";
          ...
          <div className="flex-1">
            <div className="flex items-center justify-end mb-6">
              <button className="flex items-center gap-2 text-sm text-foreground">
                <span>Sắp xếp: {sortBy === "relevance" ? "Ph\xF9 h\u1EE3p nh\u1EA5t" : sortBy}</span>
  ...
  />
            ...
            <div className="grid grid-cols-3 gap-x-6 gap-y-10">
              {allProducts.map((product) => <ProductCard key={product.id} {...product} />)}
            </div>
